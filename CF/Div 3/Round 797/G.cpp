...
int main(){
    ...
    while(t--){
        ...
        segtree_range<segnode>seg(a);
        int ans = 0, last = 0;
        for(int i=0;i<n;i++){
            if(i < last) continue;
            int u = seg.query(i, i+1).get();
            ans++;
            int c = seg.find_right(i, &segnode::go, u-1) - 1;
            seg.update(i, c, &segnode::assign, u);
            last = c;
        }
        while(q--){
            int k, d;
            ...
            k--;
            a[k] -= d;
            int u = seg.query(k, k+1).get();
            if(a[k] >= u){
                cout<<ans<<" ";
                continue;
            }
            int c = seg.find_right(0, &segnode::go, u) - 1;
            if(c == k) ans--;
            int hi = k;
            while(hi < n){
                ans--;
                hi = seg.find_right(hi, &segnode::go, u-1) - 1;
                if(hi < n){
                    u = seg.query(hi, hi+1).get();
                    if(u < a[k]) break;
                }
            }
            ans += 2;
            seg.update(k, hi, &segnode::assign, a[k]);
            cout<<ans<<" ";
        }
        cout<<"\n";
    }
